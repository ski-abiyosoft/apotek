...
class XLSXWriter
{
	...
	public function writeSelfFormatSheetRow($sheet_name, array $row, $row_options=null)
	{
		if (empty($sheet_name))
			return;
		...
		foreach ($row as $v) {
            ...
            if (preg_match('/\d{4}-\d{2}-\d{2}/', $v)){
			    ...
			    $number_format_type = 'n_datetime';
                $cell_style_idx = $this->addCellStyle($number_format, $style_string=null);
            }else if (preg_match('/[a-zA-Z]/', $v) != 1 AND is_numeric($v)){
                $number_format = '#,##0.00';
			    $number_format_type = 'n_numeric';
                $cell_style_idx = $this->addCellStyle($number_format, $style_string=null);
            }
			$this->writeCell($sheet->file_writer, $sheet->row_count, $c, $v, $number_format_type, $cell_style_idx);
			$c++;
