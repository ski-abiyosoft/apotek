...
$(document).ready(function() {
    ...
    table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('kasir_konsul/ajax_list/1')?>",
            "type": "POST"
        },
...
function filterdata(){	
	...
	table.ajax.url("<?php echo base_url('kasir_konsul/ajax_list/')?>"+str).load();	 
}
...
function Batalkan(id) {		
	swal({
		...
		$.ajax({
			...
			dataType : "json",
			data : {alasan : alasan},
			url : '<?php echo base_url()?>kasir_konsul/pembatalan/'+id,
...
function send_wa(id, hp){	 
  swal({
		...
		$.ajax({
			...
			dataType : "json",
			data : {id : id},
			url : '<?php echo base_url()?>kasir_konsul/send_wa',
			success:function(response){
				if(response.status == 1) {
					swal(
						'Sending!',
...
function send_email(id, email){	 
  swal({
		...
		$.ajax({
			...
			dataType : "json",
			data : {id : id},
			url : '<?php echo base_url()?>kasir_konsul/send_email',
			success:function(response){
				if(response.status == 1) {
					swal(
						'Sending!',
...
function cekisiobat(kwitansi, noreg){

		$.ajax({
			url: "/kasir_konsul/cek_isiobat/",
			type: "POST",
			dataType: "JSON",
			data: {kwi : kwitansi, regg : noreg},
			success: function(data){
				if(data.status == 0){
					return;
				} else {
					_urlcetak2(kwitansi, noreg);
				}
			}
		});
}

function _urlcetak( kwitansi, noreg)
{	
	var baseurl = "<?php echo base_url()?>";	
    var var1=baseurl+'kasir_konsul/cetak/?kwitansi='+kwitansi+'&noreg='+noreg;	
	window.open(var1,'_blank');
	cekisiobat(kwitansi, noreg);
	// _urlcetak2( kwitansi, noreg);
}

function _urlcetak2( kwitansi, noreg)
{	
	var baseurl = "<?php echo base_url()?>";	
	var var2=baseurl+'kasir_konsul/cetak2/?kwitansi='+kwitansi+'&noreg='+noreg;	
	var var2=baseurl+'kasir_konsul/ctk_obt/?kwitansi='+kwitansi+'&noreg='+noreg;	
