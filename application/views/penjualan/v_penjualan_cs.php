    ...
    <link href="<?php echo base_url('assets/bootstrap-datepicker/css/bootstrap-datepicker3.min.css')?>" rel="stylesheet">
			...
			<div class="row">
				<div class="col-md-12">
					<h3 class="page-title">
					  Penjualan <small>Data Customer</small>
					</h3>
...
$(document).ready(function() {
    ...
    table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('penjualan_cs/ajax_list')?>",
            "type": "POST"
        },
...
function edit_data(id)
{
    ...
    $.ajax({
        url : "<?php echo site_url('penjualan_cs/ajax_edit/')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
			...
			$('[name="bataskredit"]').val(data.bataskredit);
			$('[name="ktp"]').val(data.ktp);
			$('[name="sales"]').val(data.kodesales);
...
function save()
{
    ...
    if(save_method == 'add') {
        ...
        url = "<?php echo site_url('penjualan_cs/ajax_update')?>";
    }
...
{
    if(confirm('Yakin data Customer dengan kode '+id+' ini akan dihapus ?'))
    {
        // ajax delete data to database
        $.ajax({
            url : "<?php echo site_url('penjualan_cs/ajax_delete')?>/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
...
function cetaklap(str) {
  ...
  xhttp.open("GET", "<?php echo base_url(); ?>penjualan_cs/cetak/"+str, true);  
  xhttp.send();
}
...
<script>
	$(document).ready(function() {
		...
		$("#simkeureport").html('<iframe src="<?php echo base_url();?>penjualan_cs/cetak/'+no_daftar+'" frameborder="no" width="100%" height="420"></iframe>');
		});	
	});
