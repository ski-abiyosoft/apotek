    ...
    <link href="<?php echo base_url('assets/bootstrap-datepicker/css/bootstrap-datepicker3.min.css')?>" rel="stylesheet">
			...
			<div class="row">
				<div class="col-md-12">
					<h3 class="page-title">
					  Penjualan <small>Data Sales</small>
					</h3>
...
$(document).ready(function() {
    ...
    table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('penjualan_sl/ajax_list')?>",
            "type": "POST"
        },
...
function edit_data(id)
{
    ...
    $.ajax({
        url : "<?php echo site_url('penjualan_sl/ajax_edit/')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
            $('[name="id"]').val(data.id_sales);            
			$('[name="telp"]').val(data.telpon);            
...
function save()
{
    ...
    if(save_method == 'add') {
        ...
        url = "<?php echo site_url('penjualan_sl/ajax_update')?>";
    }
...
{
    if(confirm('Yakin data Sales dengan kode '+id+' ini akan dihapus ?'))
    {
...
function cetaklap(str) {
  ...
  xhttp.open("GET", "<?php echo base_url(); ?>penjualan_sl/cetak/"+str, true);  
  xhttp.send();
}
...
<script>
	$(document).ready(function() {
		...
		$("#simkeureport").html('<iframe src="<?php echo base_url();?>penjualan_sl/cetak/'+no_daftar+'" frameborder="no" width="100%" height="420"></iframe>');
		});	
	});
