						...
						<div class="tab-content">
							<div class="tab-pane active" id="tab1">		
														   ...
														   <label class="col-md-3 control-label">Penjamin<font color="red">*</font></label>
															<div class="col-md-9">
																<select id="cust_id" name="cust_id" class="form-control select2_el_penjamin" data-placeholder="Pilih..." onkeypress="return tabE(this,event)">
																	<option value="<?= $rowh->cust_id;?>"><?= $rowh->cust_id;?></option>
            													</select>																													    													            															
															</div>
														   </div>
													</div>	
												...
												<div class="row">	
														   ...
														   <label class="col-md-3 control-label">Kas/Bank<font color="red">*</font></label>
															<div class="col-md-9">
																 <select id="accountno" name="accountno" class="form-control select2_el_kasbank" data-placeholder="Pilih..." onkeypress="return tabE(this,event)">
															        <option value="<?= $rowh->accountno;?>"><?= $rowh->accountno." | ".$rowh->acname;?></option>
															     </select>																														    													            															
															</div>
														   </div>
													</div>												
													<div class="col-md-6">
                                                         <div class="form-group">
													        ...
													        <div class="col-md-9">
														        <input id="ardate" name="ardate" class="form-control rightJustified" type="date" value="<?php echo date('Y-m-d',strtotime($rowh->ardate)); ?>" />
												...
												<div class="row">
													...
													<div class="col-md-6">
														<div class="form-group">
																...
																<select name="jenisbayar" id="jenisbayar" class="form-control">
																	...
																	<?php
																		foreach($jenis_pembayaran as $key){
																			$selected = '';
																			if($key->id == $rowh->jenisbayar) $selected = 'selected'; 
															...
															<div class="col-md-6">
																 <input type="text" class="form-control rightJustified" name="cek_no"  id="cek_no" value="<?php echo $rowh->cek_no; ?>" >																																																																												    													            
															</div>
														</div>
														<div class="form-group">
															<label class="col-md-3 control-label">Jumlah Bayar<font color="red">*</font></label>
													        <div class="col-md-9">
                                                                 <input type="text" class="form-control rightJustified" name="totalterima_view"  id="totalterima_view" value="<?php echo number_format($rowh->totalterima,2,'.',','); ?>" readonly>	
																 <input type="hidden" class="form-control rightJustified" name="totalterima"  id="totalterima" value="<?php echo $rowh->totalterima; ?>" readonly>	
													...
													<div class="col-md-6">
														<div class="form-group">
															...
															<div class="col-md-9">
																<textarea id="keterangan" name="keterangan" class="form-control" rows="4" cols="50"><?php echo $rowh->keterangan; ?></textarea>
															</div>
														</div>
													</div>
												...
												<table id="tabel_tagihan_pilihan" 
													...
													<tbody id="body_tagihan_pilihan">
														<?php 
															foreach($detil as $value){
														?>		
															<tr>		
																<td style='text-align: center'><?php echo $value->invoiceno; ?></td>
																	<input type='hidden' id='invoiceno[]' name='invoiceno[]' value='<?php echo $value->invoiceno; ?>'>
																<td style='text-align: center'><?php echo date('d/m/Y',strtotime($value->tgltagihan)); ?></td>
																	<input type='hidden' id='tglposting[]' name='tglposting[]' value='<?php echo $value->tgltagihan; ?>'>
																<td style='text-align: center'><?php echo ($value->fakturno == null || $value->fakturno == 'null' ? '' : $value->fakturno); ?></td>
																	<input type='hidden' id='fakturno[]' name='fakturno[]' value='<?php echo $value->fakturno; ?>'>
																<td style='text-align: center'><?php echo $value->namapas; ?></td>
																	<input type='hidden' id='namapas[]' name='namapas[]' value='<?php echo $value->namapas; ?>'> 
																	<input type='hidden' id='noreg[]' name='noreg[]' value='<?php echo $value->noreg; ?>'>
																	<input type='hidden' id='rekmed[]' name='rekmed[]' value='<?php echo $value->rekmed; ?>'>
																	<input type='hidden' id='lunas[]' name='lunas[]' value='<?php echo $value->lunas; ?>'>
																<td style='text-align: right'><?php echo number_format($value->jumlahtagihan,2,'.',','); ?></td>
																	<input type='hidden' id='jumlahhutang[]' name='jumlahhutang[]' value='<?php echo $value->jumlahtagihan; ?>'>
																<td style='text-align: center'><?php echo number_format($value->jumlahbayar,2,'.',','); ?></td>
																	<input type='hidden' id='jumlahbayar[]' name='jumlahbayar[]' value='<?php echo $value->jumlahbayar; ?>'>
																<td style='text-align: center'><?php echo number_format($value->diskonrp); ?></td>
																	<input type='hidden' id='diskontotal[]' name='diskontotal[]' value='<?php echo $value->diskonrp; ?>'>
																<td style='text-align: center'><center><select id='akundiskon[]' name='akundiskon[]' class='form-control select2_el_akundiskonadjust' style="width: 247px;" data-placeholder='Pilih...' ><option value='<?php echo $value->daccountno; ?>'><?php echo $value->daccountno." | ".$value->acname; ?></option></select></center></td>
															</tr>
														<?php
															}
														?>
							  ...
							  <div class="well">
								<table>					  
								  <tr>
									...
									<td width="59" align="right"><strong><span id="_vtotal"><?php echo number_format($rowh->totalterima,2,'.',','); ?></span></strong></td>
								  </tr>
...
{	        
    ...
    if(
		...
		$.ajax({				
			url:'<?php echo site_url('penjualan_penerimaan/save/2')?>',				
