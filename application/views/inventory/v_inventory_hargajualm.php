...
tbody.scrollContent tr.alternateRow td {
	...
	padding: 2px 3px 3px 4px
			...
			<div class="row">
				<div class="col-md-12">
					<h3 class="page-title">
					  Inventory <small>Seting Harga Jual Manual</small>
					</h3>
							...
							<table id="table" class="table table-striped table-bordered" cellspacing="0" width="100%" >
                               <thead>
                                     <tr>
                                         ...
                                         <th style="text-align: center">Hrg. Retail</th>                                                                                  
										 <th style="text-align: center">Hrg. Pemborong</th>
                                         <th style="text-align: center">Hrg. Toko</th>                                                                                  										 
...
$(document).ready(function() {
    ...
    table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('inventory_hrgjualm/ajax_list')?>",
            "type": "POST"
        },
...
function edit_data(id)
{
    ...
    $.ajax({
        url : "<?php echo site_url('inventory_hrgjualm/ajax_edit/')?>/" + id,
        type: "GET",
        dataType: "JSON",
...
function save()
{
    ...
    if(save_method == 'add') {
        url = "<?php echo site_url('inventory_hrgjualm/ajax_add')?>";
    } else {
        url = "<?php echo site_url('inventory_hrgjualm/ajax_update')?>";
    }
...
function delete_data(id)
{
    if(confirm('Yakin data barang dengan kode '+id+' ini akan dihapus ?'))
    {
        ...
        $.ajax({
            url : "<?php echo site_url('inventory_hrgjualm/ajax_delete')?>/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
...
function cetaklap(str) {
  ...
  xhttp.open("GET", "<?php echo base_url(); ?>inventory_hrgjualm/cetak/"+str, true);  
  xhttp.send();
}
...
function Run() {
  ...
  xhttp.open("GET", "<?php echo base_url(); ?>inventory_hrgjualm/getitem/"+str, true);  
  xhttp.send();
}
...
<script>
	$(document).ready(function() {
		...
		$("#simkeureport").html('<iframe src="<?php echo base_url();?>inventory_hrgjualm/cetak/'+no_daftar+'" frameborder="no" width="100%" height="420"></iframe>');
		});	
	});
                    ...
                    <div class="form-body">
						...
						<table class="table">
						   ...
						   <tr>
							   ...
							   <td width="10"><button type="button" onclick="Run()" class="btn btn-success">RUN</button></td>
							   <td>
							   </td>
						   </tr>
