    ...
    <link href="<?php echo base_url('assets/bootstrap-datepicker/css/bootstrap-datepicker3.min.css')?>" rel="stylesheet">
			...
			<div class="row">
				<div class="col-md-12">
					<h3 class="page-title">
					  Inventory <small>Data Rak</small>
					</h3>
...
$(document).ready(function() {
    ...
    table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('inventory_rak/ajax_list')?>",
            "type": "POST"
        },
...
function edit_data(id)
{
    ...
    $.ajax({
        url : "<?php echo site_url('inventory_rak/ajax_edit/')?>/" + id,
        type: "GET",
        dataType: "JSON",
...
function save()
{
    ...
    if(save_method == 'add') {
        url = "<?php echo site_url('inventory_rak/ajax_add')?>";
    } else {
        url = "<?php echo site_url('inventory_rak/ajax_update')?>";
    }
...
function delete_data(id)
{
    if(confirm('Yakin data kategori dengan kode '+id+' ini akan dihapus ?'))
    {
        ...
        $.ajax({
            url : "<?php echo site_url('inventory_rak/ajax_delete')?>/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
...
function cetaklap(str) {
  ...
  xhttp.open("GET", "<?php echo base_url(); ?>inventory_rak/cetak/"+str, true);  
  xhttp.send();
}
...
<script>
	$(document).ready(function() {
		...
		$("#simkeureport").html('<iframe src="<?php echo base_url();?>inventory_rak/cetak/'+no_daftar+'" frameborder="no" width="100%" height="420"></iframe>');
		});	
	});
