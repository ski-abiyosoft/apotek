...
<style>	
	...
	thead.fixedHeader th {background:#C96;border-left:1px solid #EB8;border-right:1px solid #B74;border-top:1px solid #EB8;	padding:4px 3px;text-align: eft}
					...
					<div class="portlet">
						...
						<div class="portlet-body">
							<div class="table-toolbar">
								<div class="btn-group">
								    <a href="<?php echo base_url()?>permohonan_log/entri" class="btn btn-success">
							...
							<table id="table" class="table table-striped table-bordered" cellspacing="0" width="100%">							
                               <thead>
                                     <tr class="page-breadcrumb breadcrumb">
									     ...
									     <th class="title-white" style="text-align: center;width:11%">No. Permohonan</th>
									     <th class="title-white" style="text-align: center;width:8%">Tanggal</th>
...
$(document).ready(function() {
    ...
    table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('permohonan_log/ajax_list/1')?>",
            "type": "POST"
        },
...
function delete_data(id){
    if(confirm('Yakin data barang dengan kode '+id+' ini akan dihapus ?')){
        ...
        $.ajax({
            url : "<?php echo site_url('permohonan_log/ajax_delete')?>/"+id,
            type: "POST",
            dataType: "JSON",
...
function filterdata(){
	let tgl1    = document.getElementById("tanggal1").value;
	let tgl2    = document.getElementById("tanggal2").value;
	var id      = 2; 
	var str     = id+'~'+tgl1+'~'+tgl2; 
    // var periode = $(".caption").html();
	table.ajax.url("<?php echo base_url('permohonan_log/ajax_list/')?>"+str).load();
    const tglex1  = tgl1.split("-");
    const tglex2  = tgl2.split("-");

    $(".caption").html("Daftar Permohonan Barang - Periode "+ changemonth(tglex1[1]) +" "+ tglex1[0] +" - "+ changemonth(tglex2[1]) +" "+ tglex2[0]);		
}
