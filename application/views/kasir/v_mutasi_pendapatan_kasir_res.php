...
<style>
    ...
    <div class="additional-toolbar">
        ...
        <h4 class="caption text-success">
            <b>Mutasi Pendapatan Tunai <?= $this->input->get("cashier"); ?> Periode <?= date("d/m/Y", strtotime($this->input->get("from"))) ?> s/d <?= date("d/m/Y", strtotime($this->input->get("to"))) ?></b> 
        </h4>

    </div>

    <form id="mkform" class="form-horizontal" method="POST">
        <input type="hidden" name="nomutasi" id="nomutasi">
        <input type="hidden" name="uidlog" value="<?= $uidlog ?>">
        <input type="hidden" name="shift" value="<?= $shift ?>">
        <input type="hidden" name="fromdate" value="<?= $this->input->get("from") ?>">
        <input type="hidden" name="todate" value="<?= $this->input->get("to") ?>">
                ...
                <div class="row" style="margin:0 0 20px 0">
                    ...
                    <div class="col-sm-6">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-9">
                                <textarea type="text" class="form-control" id="keterangan" name="keterangan" rows="3" style="resize:none">Mutasi Pendapatan Kasir Tunai <?= $username ?> Shift <?= $shift ?> <?= date("d/m/Y") ?></textarea>
                            </div>
                        </div>
                        <div class="form-group">
                            <div class="col-md-4">
...
<script>
    ...
    $("#cetak_mutasi").on("click", function(){
        ...
        location.href="/mutasi_pendapatan_kasir";
        window.open("/mutasi_pendapatan_kasir/cetak/"+nomutasi, "_blank");
    });
...
<script>
    ...
    $("#save_mutasi").on("click", function(){
        var post_form = $("#mkform").serialize();
        $.ajax({
            url: "/mutasi_pendapatan_kasir/save",
            type: "POST",
            data: post_form,
            success: function(data){
                ...
                if(data.status == 0){
                    ...
                    $("#saldoawal").prop("readonly", true);
                    $("#selisih").prop("readonly", true);
    ...
    function sequentNomutasi(){
        $.ajax({
            url: "/mutasi_pendapatan_kasir/get_last_number/",
            type: "GET",
            dataType: "JSON",
            success: function(data){
    ...
    function secondNomutasi(){
        $.ajax({
            url: "/mutasi_pendapatan_kasir/get_recent_number/",
            type: "GET",
            dataType: "JSON",
            success: function(data){
