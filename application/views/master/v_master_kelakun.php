			...
			<div class="row">
				<div class="col-md-12">
                    ...
                    <span class="title-unit">
                      ...
                      - 
                      <span class="title-web">Master <small>Kelompok Perkiraan</small>
					</h3>
...
$(document).ready(function() {
    ...
    table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('master_kelakun/ajax_list')?>",
            "type": "POST"
        },
...
function edit_data(id)
{
    ...
    $.ajax({
        url : "<?php echo site_url('master_kelakun/ajax_edit/')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
		    ...
		    $('[name="id"]').val(data.id);
            $('[name="kode"]').val(data.actype);
            $('[name="nama"]').val(data.typename);
			$('[name="lap"]').val(data.ackode);
			$('[name="pls"]').val(data.pls);
...
function save()
{
    ...
    if(save_method == 'add') {
        url = "<?php echo site_url('master_kelakun/ajax_add')?>";
    } else {
        url = "<?php echo site_url('master_kelakun/ajax_update')?>";
    }
...
function delete_data(id)
{
    if(confirm('Yakin data Bank dengan kode '+id+' ini akan dihapus ?'))
    {
        ...
        $.ajax({
            url : "<?php echo site_url('master_kelakun/ajax_delete')?>/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h3 class="modal-title">Data Kelompok Akun</h3>
            </div>
            <div class="modal-body form">
                <form action="#" id="form" class="form-horizontal">
                    ...
                    <div class="form-body">
                       ...
                       <div class="form-group">
                            <label class="control-label col-md-4">Laporan </label>
                            <div class="col-md-8">
                                <select name="lap" class="form-control">
								  <option value="">--- Pilih --- </option>
								  <?php
								    foreach($actype as $row){ ?>
									  <option value="<?= $row->ackode; ?>"><?= $row->acgol; ?></option>
									<?php } 
								  ?>
								</select>
                                <span class="help-block"></span>
                        ...
                        <div class="form-group">
                            <label class="control-label col-md-4">Kelompok Laporan Rugi/Laba </label>
                            <div class="col-md-8">
                                <select name="pls" class="form-control">
								  <option value="">--- Pilih --- </option>
								  <option value="+">Pendapatan</option>
								  <option value="-">Biaya</option>
								  <option value="x">Pendapatan Diluar Usaha</option>
								  <option value="y">Beban Diluar Usaha</option>
								  <option value="x">HPP</option>
								</select>
                                <span class="help-block"></span>
                            </div>
                        </div>	
