            ...
            <tbody>
              <tr>
                ...
                <td><input name="qty[]" id="qty1" type="text" class="form-control" value="1" min="1"
                    onkeyup="total_1(1)" onchange="cekqty(1)">
                </td>
...
function getbarang(param) {
    ...
    var gud = $("#gudang_tujuan").val();
    $.ajax({
      url: "<?= site_url('Farmasi_pbb/cekbarang?gud='); ?>" + gud + "&kodebarang=" + kodebarang,
      type: "POST",
      dataType: "JSON",
      success: function(data) {
        if (data.status == 1) {
          swal({
            title: "KOSONG",
            html: "Kode Barang   : " + kodebarang + "<br>Di Gudang " + data.gudangx + " : Kosong",
            type: "info",
            confirmButtonText: "OK"
          }).then((value) => {
            $("#kode" + param).empty();
  ...
  function cekqty(param) {
    ...
    $.ajax({
      ...
      dataType: "JSON",
      success: function(data) {
        var qty_data = data.qty;
        if (qty > qty_data) {
          swal({
            title: "KUANTITAS",
            html: "Melebihi saldo akhir<br>Saldo akhir saat ini : " + separateComma(qty_data),
            type: "info",
            confirmButtonText: "OK"
          }).then((value) => {
            $("#kode" + param).empty();
