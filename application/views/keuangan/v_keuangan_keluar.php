...
$(document).ready(function() {
    ...
    // $("#simkeureport").html('<iframe src="<?php echo base_url();?>keuangan_keluar/cetak/'+sessionStorage.getItem('idCetakK_keluar')+'" frameborder="no" width="100%" height="520"></iframe>');
    ...
    table = $('#table').DataTable({
        ...
        "ajax": {
            "url": "<?php echo site_url('keuangan_keluar/ajax_list/1')?>",
            "type": "POST"
        },
...
function convert_excel(type, fn, dl) {
    ...
    return dl ?
        ...
        XLSX.writeFile(wb, fn || ('Keuangan-Keluar.' + (type || 'xlsx')));
}
...
function id_cetak(id) {
    $("#simkeureport").html('<iframe src="<?php echo base_url();?>keuangan_keluar/cetak/' + id +
...
function delete_data(id, nomor) {
    swal({
        title: 'KAS/BANK',
        html: "Pengeluaran ini akan dihapus ? <strong><p>" + nomor,
        type: 'warning',
        ...
        $.ajax({
            type: 'POST',
            dataType: "json",
            url: '<?php echo base_url('keuangan_keluar')?>/ajax_delete',
            data: {
                id: id
            },
            success: function(response) {
                if (response.status == '1') {
                    swal(
                        '',
                        'Pengeluaran Kas sudah dihapus...',
                        ''
                    )
                } else {
                    swal(
                        '',
                        'Pengeluaran Kas dihapus.',
                        ''
                    )
                }
...
function filterdata() {
    ...
    table.ajax.url("<?php echo base_url('keuangan_keluar/ajax_list/')?>" + str).load();
}
    </script>