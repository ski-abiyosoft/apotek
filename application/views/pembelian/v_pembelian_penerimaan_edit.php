                            ...
                            <div class="row">
                                ...
                                <div class="col-md-7">
                                    <div class="form-group">
                                        ...
                                        <div class="col-md-9">
                                            <input class="form-control" type="text" name="nomorpo" id="nomorpo" value="<?= $header->po_no; ?>" readonly>
                                        </div>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-7">
                                    <div class="form-group">
                                        ...
                                        <div class="col-md-4">
                                            <select id="gudang" name="gudang" class="form-control select2_el_logistik_depo" data-placeholder="Pilih..." onkeypress="return tabE(this,event)">
                                                <?php
                                                if ($header->gudang) {
                    ...
                    <div class="row">
                        <div class="col-xs-9">
                            <div class="wells">
                                ...
                                <div class="btn-group">
                                    <a class="btn red" href="<?php echo base_url('logistik_bapb/') ?>"><i class="fa fa-undo"></i><b> KEMBALI </b></a>
                                </div>
...
<script>
    ...
    function tambah() {
        ...
        td2.innerHTML = "<input name='qty[]'    id=qty" + idrow + " onchange='totalline(" + idrow +
            "); cekqty(" + idrow + "); changeqty(" + idrow + ")' value='1'  type='text' class='form-control rightJustified'  >";
        ...
        td6.innerHTML = "<input name='discrp[]'   id=discrp" + idrow + " onchange='totalline(" + idrow +
            ")' value='0'  type='text' class='form-control rightJustified'  >";
        td7.innerHTML = "<input type='checkbox' name='tax[]'    id=tax" + idrow + " onchange='totalline(" + idrow +
            ")' class='form-control'>";
    ...
    function changeqty(id) {
        ...
        $.ajax({
            url: "<?= site_url('logistik_bapb/data_awal_terima/?kode=') ?>" + kode + "&gudang=" + gudang + "&terima_no=" + terima_no,
            type: "GET",
            dataType: "JSON",
    ...
    function cekdiscrp(id) {
        ...
        $('#discrp' + id).val(separateComma(discrp.toFixed(0)));
        $('#jumlah' + id).val(separateComma(jumlah_new.toFixed(0)));
    ...
    function cekharga(id) {
        ...
        var discx = parseInt(disc.replaceAll(',', ''));
                ...
                if (data.status == 1) {
                    swal({
                        ...
                        for (var i = 1; i < rowCount; i++) {
                            ...
                            diskon = eval((diskonb / 100) * jumlahb * hargab);
                            if (document.getElementById('tax' + i).checked === true) {
                                tppn += (qtyb * hargab - diskonrpb) * cekppn2;
                            }
                        }
                        var dsc = (data.harga * qty) * diskonb / 100;
                        $('#jumlah' + id + '').val(separateComma(jumlah_new.toFixed(0)));
                        $('#discrp' + id + '').val(separateComma(dsc.toFixed(0)));
                        $('#disc' + id + '').val(separateComma(dsc.toFixed(0)));
                        jumtot = data.harga * qty - dsc;
                        console.log(dsc)
                        $('#jumlah' + id + '').val(separateComma(jumtot.toFixed(0)));
                        // ppn = subtot * cekppn2;
                        tot = subtot + tmaterai - diskonnet + tppn;
                        // console.log(subtot);
                        document.getElementById("_vdiskon").innerHTML = separateComma(diskonnet.toFixed(
                            0));
                        document.getElementById("_vsubtotal").innerHTML = separateComma(subtot.toFixed(
                            0));
                        document.getElementById("_vppn").innerHTML = separateComma(tppn.toFixed(0));
                        document.getElementById("_vtotal").innerHTML = separateComma(tot.toFixed(0));
                    });
                } else if (data.status == 2) {
                    $('#harga' + id + '').val(separateComma(hargax.toFixed(0)));
                    tqty = 0;
                    tjumlah = 0;
                    tdiskon = 0;
                    diskonnet = 0;
                    ...
                    for (var i = 1; i < rowCount; i++) {
                        ...
                        tot = hargax * qtyx - dsc;
                        $('#discrp' + id + '').val(formatCurrency1(0));
                        $('#disc' + id + '').val(separateComma(dsc.toFixed(0)));
                        var discxe = $('#discrp' + id + '').val(separateComma(dsc));
                        $('#jumlah' + id + '').val(formatCurrency1(tot));
                        diskon = eval((diskonb / 100) * jumlahb * hargab);
                    }
                    if (document.getElementById('tax' + id).checked === true) {
                        tot_1 = (qtyx * hargax - (qtyx * hargax * discx / 100));
                        tppn += (tot_1 * cekppn2);
                    }
                    ...
                    var tot = subtot - diskonnet1 + tmaterai + tppn;
    ...
    function showbarangname(str, id) {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $('#jumlah' + vid).val(separateComma(harga.toFixed(0)));
                $('#po' + vid).val(nomorpo);
                totalline(vid);
            }
        });
    ...
    function save() {
        ...
        if (nomor == "" || total == "0.00" || total == "" || supp == null || gudang == null) {
            ...
            $.ajax({
                url: "<?php echo site_url('logistik_bapb/save/2') ?>",
                data: $('#frmpembelian').serialize(),
                type: 'POST',
                success: function(data) {
    ...
    function totalline(id) {
        ...
        for (var i = 1; i < rowCount; i++) {
            ...
            jumlahz = jumlah1 * harga1 - diskon2;
        ...
        //    row.cells[3].children[0].value = separateComma(harga);
        //    row.cells[5].children[0].value = separateComma(diskon);
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    for (i = 0; i <= data.length - 1; i++) {
                        ...
                        document.getElementById("qty" + x).value = data[i].qty_terima;
