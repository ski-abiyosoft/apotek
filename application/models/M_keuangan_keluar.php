...
class M_keuangan_keluar extends CI_Model {
	...
	var $table = 'tbl_hbayar';
	var $column_order = array('id', 'koders','bayarno','accountno','keterangan','tglbayar','jmbayar','username',null); 
	var $column_search = array('id', 'koders','bayarno','accountno','keterangan','tglbayar','jmbayar','username'); 
	...
	{   
	   ...
	   if($jns==1){
			$tanggal = date('Y-m-d');
			$this->db->select('tbl_hbayar.*')->from('tbl_hbayar');
			// $this->db->where(array('tglbayar' => $tanggal));
			$this->db->where(array('tglbayar >=' => $tanggal,'koders' => $unit));
			$this->db->order_by('tglbayar, bayarno');
		} else {
		    $this->db->select('tbl_hbayar.*')->from('tbl_hbayar');
			// $this->db->where(array('tglbayar >=' => $bulan,'tglbayar<= ' => $tahun));
			$this->db->where(array('tglbayar >=' => $bulan,'tglbayar<= ' => $tahun,'koders' => $unit));
			$this->db->order_by('tglbayar, bayarno');
