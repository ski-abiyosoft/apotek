...
class M_farmasi_expire extends CI_Model
{


	var $table = 'tbl_apohex';
	var $column_order = array('id', 'koders', 'ed_no', 'tgl_ed', 'gudang', 'keterangan', 'approved', 'approve_1', 'approve_2', 'approve_3', null);
	var $column_search = array('id', 'koders', 'ed_no', 'tgl_ed', 'gudang', 'keterangan', 'approved', 'approve_1', 'approve_2', 'approve_3');
	...
	private function _get_datatables_query($jns, $bulan, $tahun)
	{
		...
		if ($jns == 1) {
			$tanggal = date('Y-m-d');
			$this->db->select('tbl_apohex.*')->from('tbl_apohex');
			$this->db->where(array('tgl_ed' => $tanggal));
			$this->db->where(array('koders' => $unit));
			$this->db->order_by('tgl_ed, ed_no');
		} else {
			$this->db->select('tbl_apohex.*')->from('tbl_apohex');
			$this->db->where(array('tgl_ed >=' => $bulan, 'tgl_ed<= ' => $tahun));
			$this->db->where(array('koders' => $unit));
			$this->db->order_by('tgl_ed, ed_no');
		}
	...
	public function count_all($jns, $bulan, $tahun)
	{
		if ($jns == 1) {
			$this->db->select('tbl_apohex.*')->from('tbl_apohex');
			$this->db->where(array('year(tgl_ed)' => $tahun, 'month(tgl_ed)' => $bulan));
		} else {
			$this->db->select('tbl_apohex.*')->from('tbl_apohex');
			$this->db->where(array('tgl_ed >=' => $bulan, 'tgl_ed<= ' => $tahun));
		}
