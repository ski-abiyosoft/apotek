...
class M_farmasi_produksi extends CI_Model {
   
	var $table = 'tbl_apohproduksi';
	var $column_order = array('id', 'koders','prdno','tglproduksi','gudang','kodebarang','keterangan',null); 
	var $column_search = array('id', 'koders','prdno','tglproduksi','gudang','kodebarang','keterangan'); 
	...
	public function list(){
		...
		return $this->db->query("SELECT a.id, a.koders, a.username, a.prdno, a.tglproduksi, a.kodebarang, a.gudang 
		FROM tbl_apohproduksi AS a
		WHERE a.koders = '$unit'
		AND a.tglproduksi LIKE '%$date%'");
	}

	public function list_by_date($from, $to){
		$unit = $this->session->userdata("unit");
		$date = date("Y-m-d");
		...
		FROM tbl_apohproduksi AS a
		WHERE a.koders = '$unit'
		AND a.tglproduksi BETWEEN '$from' AND '$to'");
	}
	...
	// 	if($jns==1){
	// 		$tanggal = date('Y-m-d');
	// 		$this->db->select('tbl_apohproduksi.*')->from('tbl_apohproduksi');
	// 		$this->db->where(array('tglproduksi' => $tanggal));
	// 		$this->db->where(array('koders' => $unit));
	// 		$this->db->order_by('tglproduksi, prdno');
	// 	} else {
	// 	    $this->db->select('tbl_apohproduksi.*')->from('tbl_apohproduksi');
	// 		$this->db->where(array('tglproduksi >=' => $bulan,'tglproduksi<= ' => $tahun));
	// 		$this->db->where(array('koders' => $unit));
	// 		$this->db->order_by('tglproduksi, prdno');					
	// 	}
	...
	// 	if($jns==1){
	// 		$this->db->select('tbl_apohproduksi.*')->from('tbl_apohproduksi');
	// 		$this->db->where(array('year(tglproduksi)' => $tahun,'month(tglproduksi)' => $bulan));
	// 	} else {
	// 	    $this->db->select('tbl_apohproduksi.*')->from('tbl_apohproduksi');
	// 		$this->db->where(array('tglproduksi >=' => $bulan,'tglproduksi<= ' => $tahun));
