	...
	private function _get_datatables_query( $filter ="" ){
		...
		if($filter !="")
		{
			...
			if( $kd!="" || $nm!="" || $sat!="" ){	 
				...
				if($kd !=""){	
					$this->db->like('kodebarang', $kd);			
				}
				if($nm !=""){			
					$this->db->like('namabarang', $nm);
				}
	
				if($sat !=""){	
					$this->db->like('satuan1', $sat);			
				}	
	...
	public function return_row_with_data($rowcount,$info){
		...
		?>
			...
			<td id="td_<?=$rowcount;?>_3">
				<!-- item name  -->
				<input type="text" style="font-weight: bold-;text-align:right" name="td_data_4[]" id="td_data_<?=$rowcount;?>_4" class="form-control no-padding" value='<?=number_format($item_harga,0,'.',',');?>' onchange="getMargin(this.value, '<?=$rowcount;?>')">
			</td>			   
