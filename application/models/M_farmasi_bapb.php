...
class M_farmasi_bapb extends CI_Model {
	...
	{   
		...
		$sql = " SELECT (select IFNULL(tukarfaktur,0) from tbl_apoap b where a.terima_no=b.terima_no) tukarfaktur, a.* ,userid username,(select vendor_name from tbl_vendor b where a.vendor_id=b.vendor_id)vendor_name
		FROM tbl_baranghterima a
		WHERE koders = '$unit' $tgll $search2 
		ORDER BY terima_date, terima_no, id ASC $limm ";
	...
	public function _datapo($nopo){
		...
		$q1=$this->db->query("select * from tbl_barangdpo where po_no = '$nopo' ORDER BY po_no DESC ")->result();
		if($q1){
		$rowcount =1;
		foreach ($q1 as $res1) {			
...
<tr id="row_<?=$rowcount;?>" data-row='<?=$rowcount;?>'>
    ...
    <td id="td_<?=$rowcount;?>_4">
        ...
        <input type="text" style="font-weight: bold-;text-align:right" name="td_data_<?=$rowcount;?>_4"
            ...
            value='<?=number_format($item_harga,0,'.',',');?>'>
    </td>

    <td id="td_<?=$rowcount;?>_5">
        <!-- item name  -->
        <input type="text" style="font-weight: bold-;text-align:right" name="td_data_<?=$rowcount;?>_4"
            id="td_data_<?=$rowcount;?>_5" class="form-control no-padding" value='<?=$item_diskon;?>'>
    </td>
    <td id="td_<?=$rowcount;?>_6">
        <!-- item name  -->
        <input type="checkbox" <?= ($item_tax==1?'checked':'') ?> style="font-weight: bold-;text-align:right"
            name="td_data_<?=$rowcount;?>_4" id="td_data_<?=$rowcount;?>_6" class="form-control no-padding"
            value='<?=number_format($item_tax,0,'.',',');?>'>
    </td>
    <td id="td_<?=$rowcount;?>_7">
        <!-- item name  -->
        <input type="text" style="font-weight: bold-;text-align:right" name="td_data_<?=$rowcount;?>_4"
            id="td_data_<?=$rowcount;?>_7" class="form-control no-padding"
            value='<?=number_format($item_total,0,'.',',');?>'>
    </td>
