...
class M_global extends CI_Model
{
	...
	function getdokter($str)
	{
		...
		$query = $this->db->query("(SELECT DISTINCT b.kodokter as id, b.nadokter as text
			FROM tbl_doktercabang a 
			LEFT JOIN tbl_dokter  b ON a.kodokter=b.kodokter 
			JOIN tbl_drpoli dr ON b.kodokter=dr.kodokter
			WHERE nadokter='-' AND a.koders = '$cabang'
			LIMIT 1
			)
			UNION ALL 
			(SELECT DISTINCT b.kodokter as id, b.nadokter  as text
			FROM tbl_doktercabang a 
			LEFT JOIN tbl_dokter  b ON a.kodokter=b.kodokter 
			JOIN tbl_drpoli dr ON b.kodokter=dr.kodokter
			WHERE nadokter<>'-' AND a.koders = '$cabang'
			AND (b.kodokter like '%$str%' OR b.nadokter like '%$str%')
			ORDER BY nadokter)");
