...
class M_mutasi_gudang_log extends CI_Model {

	var $table = 'tbl_apohmovelog';
	...
	private function _get_datatables_query( $jns, $bulan, $tahun )
	{
		...
		if($jns==1){
			$tanggal = date('Y-m');
			$this->db->select('tbl_apohmovelog.*')->from('tbl_apohmovelog');
			$this->db->like(array('movedate' => $tanggal));
