...
class M_pendaftaranVRS extends CI_Model {
	...
	private function _get_datatables_query($jns, $bulan, $tahun)
	{
		...
		if($jns == 1){
			$tanggal = date('Y-m-d');
			$this->db->where(['pasien_rajal.tglmasuk' => $tanggal, 'pasien_rajal.batal' => 0]);
		} else {
		    $this->db->where(['pasien_rajal.tglmasuk >=' => $bulan,'tglmasuk<= ' => $tahun]);
		}
		$i = 0;
  ...
  function get_datatables( $jns, $bulan, $tahun)
	...
	public function count_all( $jns, $bulan, $tahun )
	{
		...
		if($jns==1){
			$this->db->where(['year(tglmasuk)' => $tahun,'month(tglmasuk)' => $bulan]);
		} else {
			$this->db->where(['tglmasuk >=' => $bulan,'tglmasuk<= ' => $tahun]);
