...
class M_KartuStock extends CI_Model
{
	...
	public function tgl($cabang, $barang, $gudang, $tgl1, $tgl2, $jam, $_tanggalawal)
	{
		...
		$aposesuai = $this->db->get_where("tbl_aposesuai", ["kodebarang" => $barang, "koders" => $cabang, "gudang" => $gudang])->row();
		$jamperiode = $aposesuai->jamentry;
		if($tgl1 < $_tgl1 && $jam < $jamperiode){
		...
		$mutasi =
			"SELECT * FROM(
				...
				FROM tbl_aposesuai 
				WHERE tbl_aposesuai.type = 'adjustment'
			...
			where 
			kodebarang ='$barang' and koders ='$cabang' and
			gudang = '$gudang' $tgll and jam > '$jamperiode' ORDER BY tanggal, jam ASC";
		return $this->db->query($mutasi)->result();
	}
	...
	public function stok_awal_farmasi($cabang, $barang, $gudang, $tgl1, $tgl2)
	{
		...
		$cek2 = $this->db->query("SELECT hasilso as saldoawal, jamentry as jam FROM tbl_aposesuai WHERE koders = '$cabang' AND kodebarang = '$barang' AND gudang = '$gudang' AND tglso = '$tgl'");

		if ($cek2->num_rows() > 0) {
			$saldo = $cek2->row();
		} else {
			$date = date("H:i:S");
			$cek1 = $this->db->query("SELECT saldoawal as saldoawal, '$date' as jam FROM tbl_barangstock WHERE koders = '$cabang' AND kodebarang = '$barang' AND gudang = '$gudang'")->row();
			$saldo = $cek1;
		}
		return $saldo;
	}

	public function cekstok_farmasi($cabang, $barang, $gudang, $tgl1, $tgl2, $jamnya)
	{
