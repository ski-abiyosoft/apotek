...
class Master_golongan extends CI_Controller {
	...
	{
		parent::__construct();
		$this->load->model('M_golongan','M_golongan');
		$this->load->helper('simkeu_rpt');
		$this->session->set_userdata('menuapp', '1000');
		$this->session->set_userdata('submenuapp', '1202');
	}
	...
	public function index()
	{
		...
		if(!empty($cek))
		{
			$this->load->view('master/v_master_golongan');
		} else
		{
	...
	{
		$list = $this->M_golongan->get_datatables();
		$data = array();
		$no = $_POST['start'];
	...
	{
		$data = $this->M_golongan->get_by_id($id);		
	...
	public function ajax_add()
	{
		$this->_validate();
		$data = array(
				'apocode' => urut_transaksi('GOLONGAN_OBAT',10),
				'aponame' => $this->input->post('nama'),
				'apogroup' => 'GOLONGAN_OBAT',
				
			);
		$insert = $this->M_golongan->save($data);
	...
	public function ajax_update()
	{
		...
		$this->M_golongan->update(array('id' => $this->input->post('id')), $data);
		echo json_encode(array("status" => TRUE));
	}

	public function ajax_delete($id)
	{
		$this->M_golongan->delete_by_id($id);
		echo json_encode(array("status" => TRUE));
	}
