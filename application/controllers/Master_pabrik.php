...
class Master_pabrik extends CI_Controller {
	...
	{
		parent::__construct();
		$this->load->model('M_pabrik','M_pabrik');
		$this->load->helper('simkeu_rpt');
		$this->session->set_userdata('menuapp', '1000');
		$this->session->set_userdata('submenuapp', '1203');
	}
	...
	public function index()
	{
		...
		if(!empty($cek))
		{
			$this->load->view('master/v_master_pabrik');
		} else
		{
	...
	{
		$list = $this->M_pabrik->get_datatables();
		$data = array();
		$no = $_POST['start'];
		...
		$output = array(
						"draw" => $_POST['draw'],
						"recordsTotal" => $this->M_pabrik->count_all(),
						"recordsFiltered" => $this->M_pabrik->count_filtered(),
						"data" => $data,
				);
	...
	{
		$data = $this->M_pabrik->get_by_id($id);		
	...
	public function ajax_add()
	{
		$this->_validate();		
		$data = array(
				'apocode' => urut_transaksi('PABRIK_OBAT',10),
				'aponame' => $this->input->post('nama'),
				'apogroup' => 'PABRIK_OBAT',
				
			);
		$insert = $this->M_pabrik->save($data);
	...
	public function ajax_update()
	{
		...
		$this->M_pabrik->update(array('id' => $this->input->post('id')), $data);
		echo json_encode(array("status" => TRUE));
	}

	public function ajax_delete($id)
	{
		$this->M_pabrik->delete_by_id($id);
		echo json_encode(array("status" => TRUE));
	}
