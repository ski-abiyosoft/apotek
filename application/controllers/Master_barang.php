...
class Master_barang extends CI_Controller 
{
	...
	public function ajax_add(){
		...
		if($insert){
			...
			foreach($rs as $r) {
				...
				$this->db->insert("tbl_status_barang", $data_status);
			}
		}
	...
	public function ajax_update(){
		...
		$aktif        = $this->input->post("status");
		$cabang       = $this->session->userdata("unit");
		$this->db->query("UPDATE tbl_status_barang SET status = '$aktif' WHERE koders = '$cabang' AND kodebarang = '$kodebarang'");
		$data = [
			'kodebarang'   => $kodebarang,
			'namabarang'   => $this->input->post('nama'),
			'icgroup'      => $this->input->post("inventorycat"),
		...
		$this->M_barang->update(['id' => $this->input->post('id')], $data);
		$this->db->query("DELETE FROM tbl_barangcabang WHERE kodebarang = '$kodebarang'");
