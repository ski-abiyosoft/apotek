...
class Master_nomr_pasien extends CI_Controller {
	...
	{
		parent::__construct();
		$this->load->model('M_norm_pasien','M_norm_pasien');
	...
	{		
		$list = $this->M_norm_pasien->get_datatables( $kode );
		$data = array();
		$no = $_POST['start'];
		...
		$output = array(
						"draw" => $_POST['draw'],
						"recordsTotal" => $this->M_norm_pasien->count_all(),
						"recordsFiltered" => $this->M_norm_pasien->count_filtered( $kode ),
						"data" => $data,
				);
	...
	{
		$data = $this->M_norm_pasien->get_by_id($id);		
	...
	public function ajax_add()
	{
		...
		$insert = $this->M_norm_pasien->save($data);
	...
	public function ajax_update()
	{
		...
		$this->M_norm_pasien->update(array('id' => $this->input->post('id')), $data);
		echo json_encode(array("status" => TRUE));
	}

	public function ajax_delete($id)
	{
		$this->M_norm_pasien->delete_by_id($id);
		echo json_encode(array("status" => TRUE));
	}
