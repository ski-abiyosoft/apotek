...
class Master_costcenter extends CI_Controller {
	...
	{
		parent::__construct();
		$this->load->model('M_costcenter','M_costcenter');
		$this->load->helper('simkeu_rpt');
		$this->session->set_userdata('menuapp', '1000');
		$this->session->set_userdata('submenuapp', '1403');
	}
	...
	public function index()
	{
		...
		if(!empty($cek))
		{
			$this->load->view('master/v_master_costcenter');
		} else
		{
	...
	{
		$list = $this->M_costcenter->get_datatables();
		$data = array();
		$no = $_POST['start'];
		...
		$output = array(
						"draw" => $_POST['draw'],
						"recordsTotal" => $this->M_costcenter->count_all(),
						"recordsFiltered" => $this->M_costcenter->count_filtered(),
						"data" => $data,
				);
	...
	{
		$data = $this->M_costcenter->get_by_id($id);		
	...
	public function ajax_add()
	{
		$this->_validate();
		$data = array(
				'depid' => $this->input->post('kode'),
				'namadep' => $this->input->post('nama'),
				
			);
		$insert = $this->M_costcenter->save($data);
	...
	public function ajax_update()
	{
		...
		$this->M_costcenter->update(array('id' => $this->input->post('id')), $data);
		echo json_encode(array("status" => TRUE));
	}

	public function ajax_delete($id)
	{
		$this->M_costcenter->delete_by_id($id);
		echo json_encode(array("status" => TRUE));
	}
