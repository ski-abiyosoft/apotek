...
class Master_pekerjaan extends CI_Controller {
	...
	{
		parent::__construct();
		$this->load->model('M_pekerjaan','M_pekerjaan');
		$this->load->helper('simkeu_rpt');
		$this->session->set_userdata('menuapp', '1000');
		$this->session->set_userdata('submenuapp', '1111');
	}
	...
	public function index()
	{
		...
		if(!empty($cek))
		{
			$this->load->view('master/v_master_pekerjaan');
		} else
		{
	...
	public function ajax_add()
	{
		...
		$insert = $this->M_pekerjaan->save($data);
	...
	public function ajax_update()
	{
		...
		$this->M_pekerjaan->update(array('id' => $this->input->post('id')), $data);
		echo json_encode(array("status" => TRUE));
	}
