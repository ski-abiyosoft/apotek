	...
	public function cetak()
	{
		...
		if (!empty($cek)) {
			if ($laporan == 1) {
				...
				// 	) AS xx
				$query = $this->db->query("SELECT h.resepno, d.kodebarang, h.tglresep, h.jam, d.namabarang, d.satuan, SUM(d.qty) as qty, SUM(d.totalrp) as totalrp FROM tbl_apodresep d 
		...
		if ($laporan == '1') {
			...
			$chari .= "
                         <table style=\"border-collapse:collapse;font-family: tahoma; font-size:20px\" width=\"100%\" align=\"center\" border=\"0\">
                              <tr>
                                   <td style=\"text-align:center;\"><b>02 LAPORAN PENJUALAN OBAT</b></td>
                              </tr> 
                         </table>";
			$chari .= "
	...
	public function excel()
	{
		...
		if (!empty($cek)) {
			if ($laporan == 1) {
				...
				// ")->result();
				$query = $this->db->query("SELECT h.resepno, d.kodebarang, h.jam, d.namabarang, d.satuan, SUM(d.qty) as qty, SUM(d.totalrp) as totalrp FROM tbl_apodresep d 
				JOIN tbl_apohresep h ON d.resepno=h.resepno 
				WHERE h.koders = '$unit' AND h.tglresep BETWEEN '$dari' AND '$sampai'
				GROUP BY d.kodebarang")->result();
				$data = [
					'judul' => '02 LAPORAN PENJUALAN OBAT',
					'query' => $query,
				];
