...
class Kasir_obat extends CI_Controller
{
	...
	public function ajax_list($param)
	{
		...
		foreach ($list as $unit) {
			...
			$cek5   = $this->db->query("SELECT r.*, h.kodepel FROM tbl_apohreturjual r JOIN tbl_apohresep h ON r.resepno = h.resepno WHERE r.returno = '$dataresep->resepno'")->row();
			if($cek) {
				if($cek2) {
					if($cek2->kodepel == "adr") {
	...
	public function edit($id)
	{
		...
		if (!empty($cek)) {
			...
			$qkasir = $this->db->query("SELECT tbl_kasir.*, tbl_pasien.namapas from tbl_kasir left outer join tbl_pasien on tbl_kasir.rekmed=tbl_pasien.rekmed where tbl_kasir.id = '$id'")->row();
			$data['kasir'] = $qkasir;
			$noreg       = $qkasir->noreg;
			$kwitansi    = $qkasir->nokwitansi;
			$qresep      = $this->db->query("SELECT * from tbl_apoposting where nokwitansi = '$kwitansi'")->row_array();
			$qhresepx    = $this->db->query("SELECT * from tbl_apohresep where nokwitansi = '$kwitansi'")->row();
			if($qhresepx) {
				$qhresep = $qhresepx;
				$cek6   = "";
			} else {
				$qhresep = $this->db->query("SELECT * FROM tbl_apoposting WHERE nokwitansi = '$kwitansi'")->row();
				$cek5   = $this->db->query("SELECT r.*, h.kodepel FROM tbl_apohreturjual r JOIN tbl_apohresep h ON r.resepno = h.resepno WHERE r.returno = '$qhresep->resepno'")->row();
				$cek6   = $this->db->get_where("tbl_apodreturjual", ["returno" => $cek5->returno])->num_rows();
			}
			// uangr			
			$cek    = $this->db->get_where("tbl_hset_farma", ["koders" => $this->session->userdata("unit")])->row();
			...
			if($cek) {
				if($cek2) {
					...
					if($cek5->kodepel == "adr") {
						...
						$uangr = ($cek->uang_r * $cek6) + $uangracik;
					} else {
						$uangr = $cek->uang_r * $cek6;
					}
				}
			} else {
				$uangr = 0;
	...
	public function ajax_add_bayar()
	{
		...
		if($totalresep > 0) {
			...
			$data = array(
				...
				'namapasien' => $this->input->post('namapasien'),
				'shift' => $shift,
				'lainket' => 'Returan sesudah bayar kasir',
			);
		}
	...
	public function cekretur($noretur) {
		$cek = $this->db->query("SELECT * FROM tbl_apohreturjual WHERE returno = '$noretur'")->num_rows();
		if($cek > 0) {
	...
	public function cetak()
	{
		...
		if (!empty($cek)) {
			...
			$cek5  = $this->db->query("SELECT r.*, h.kodepel FROM tbl_apohreturjual r JOIN tbl_apohresep h ON r.resepno = h.resepno WHERE r.returno = '$noresep'")->row();
			$cek6  = $this->db->get_where("tbl_apodreturjual", ["returno" => $noresep])->num_rows();
			if($cek) {
				if($cek2) {
					if($cek2->kodepel == "adr") {
			...
			$headerx   	= $this->db->query($qheader)->row();
			if($headerx) {
				$header = $headerx;
			} else {
				$header = $this->db->query("SELECT r.*, h.* FROM tbl_apohreturjual r JOIN tbl_apohresep h ON r.resepno = h.resepno WHERE h.resepno = '$noresep'")->row();
			}
			$posting    = $this->db->query($qposting)->row_array();
			$detilx      = $this->db->query($qdetil)->result();
			if($detilx) {
				$detil = $detilx;
			} else {
				$detil = $this->db->query("SELECT r.*, r.qtyretur as qty, r.discount as discrp, (0 - r.totalrp) as totalrp, b.namabarang FROM tbl_apodreturjual r JOIN tbl_barang b ON r.kodebarang = b.kodebarang WHERE returno = '$noresep'")->result();
			}
			...
			if ($kasir->kembalikeuangmuka == 0) {
				$pdf->FancyRow([
					($kasir->kembali >= 0 ? 'Kembali ke pasien' : ''), ($kasir->kembali >= 0 ? ':' : ''), ($kasir->kembali >= 0 ? number_format($kasir->kembali) : ''), '', '', ''
				], $fc, $border, $align, $style, $size);
			} else {
	...
	public function cetak_kasir_nota()
	{
		$cek = $this->session->userdata('level');
		$unit = $this->session->userdata('unit');
		if (!empty($cek)) {
	...
	public function cetak_jaminan()
	{
		...
		if (!empty($cek)) {
			...
			$chari .= "
					...
					<thead>		
						<tr>
							<td bgcolor=\"#cccccc\" width=\"5%\" style=\"text-align:center;\">No</td>
							<td bgcolor=\"#cccccc\" style=\"text-align:center;\"><b>Penjamin</b></td> 
							<td bgcolor=\"#cccccc\" style=\"text-align:center;\"><b>Tercover Rp</b></td>
						</tr> 
			</thead>";
			$no = 1;
	...
	public function cetak_jaminan_nota()
	{
		$cek          = $this->session->userdata('level');
		$unit         = $this->session->userdata('unit');
		$user         = $this->session->userdata('username');
		...
		if (!empty($cek)) {
			...
			$terbilang = terbilang($xxx);
			$chari .= "<table style=\"border-collapse:collapse;font-family: Century Gothic; font-size:10px; color:#000;\" width=\"100%\" border=\"0\" >
			<thead>
				<tr>
				...
				<tr>
					<td colspan=\"5\">
						<tr>
								<td align=\"center\" style=\"font-size:8px;border-bottom: none;\"><b><br>$namars</b></td>
						</tr>
						<tr>
								<td align=\"center\" style=\"font-size:7px;\">$alamat</td>
						</tr>
						<tr>
								<td align=\"center\" style=\"font-size:7px;\">$alamat2</td>
						</tr>
						<tr>
								<td align=\"center\" style=\"font-size:7px;\">Wa :$whatsapp    Telp :$phone </td>
						</tr>
						<tr>
								<td align=\"center\" style=\"font-size:7px;\">No. NPWP : $npwp</td>
						</tr>
					</td>
				</tr>
			</table>";
			$chari .= "
					<table style=\"border-collapse:collapse;font-family: tahoma; font-size:5px\" width=\"100%\" align=\"center\" border=\"0\">
								<tr>
										<td> &nbsp; </td>
								</tr> 
					</table>";
			...
			<table style=\"border-collapse:collapse;font-family: Tahoma; font-size:10px\" width=\"100%\" align=\"center\" border=\"0\" cellspacing=\"1\" cellpadding=\"3\">
				<tr>
					<td width=\"15%\" style=\"text-align:center; font-size:15px;\"><b>DOKUMEN JAMINAN</b></td>
				</tr>
			</table>";
			...
			$chari .= "
					<table style=\"border-collapse:collapse;font-family: tahoma; font-size:10px\" width=\"100%\" align=\"left\" border=\"0\">     
						<tr>
							<td width=\"35%\" style=\"border-top: none;border-right: none;border-left: none;\">Sudah terima dari</td>
							<td width=\"2%\" style=\"border-top: none;border-right: none;border-left: none;\"> : </td>
							<td style=\"border-top: none;border-right: none;border-left: none;\">$kasir->dibayaroleh</td>
						</tr> 
						<tr>
			...
			$chari .= "
					<table style=\"border-collapse:collapse;font-family: tahoma; font-size:10px\" width=\"100%\" align=\"center\" border=\"0\" cellspacing=\"1\" cellpadding=\"1\">
							<tr>
								<td> &nbsp; </td>
							</tr> 
					</table>";
			$chari .= "
					...
					<thead>		
						<tr>
							<td bgcolor=\"#cccccc\" width=\"30%\" style=\"text-align:center;\">No</td>
							<td bgcolor=\"#cccccc\" width=\"40%\" style=\"text-align:center;\"><b>Penjamin</b></td> 
							<td bgcolor=\"#cccccc\" width=\"30%\" style=\"text-align:center;\"><b>Tercover Rp</b></td>
						</tr> 
			</thead>";
			$no = 1;
			foreach ($jaminan as $j) {
			...
			$chari .= "
				<table style=\"border-collapse:collapse;font-family: tahoma; font-size:10px\" width=\"100%\" align=\"center\" border=\"0\">
					<tr>
						<td>
							<table style=\"border-collapse:collapse;font-family: tahoma; font-size:10px\" width=\"50%\" align=\"left\" border=\"0\">
								<tr>
									<td> &nbsp; </td>
									<td> &nbsp; </td>
									<td> &nbsp; </td>
						...
						<td>
							<table style=\"border-collapse:collapse;font-family: tahoma; font-size:10px\" width=\"60%\" align=\"right\" border=\"0\">
								<tr>
									<td width=\"40%\" style=\"border-top: 1px solid #000;border-bottom: 1px solid #000;	\"><b>Total Rp</b></td>
									<td width=\"10%\" style=\"border-top: 1px solid #000;border-bottom: 1px solid #000;\"> : </td>
									<td width=\"50%\" style=\"text-align:right; border-top: 1px solid #000;border-bottom: 1px solid #000;\"><b>".number_format($totalnya,2)."</b></td>
								</tr>
								<tr>
									<td colspan=\"3\" style\"border-right: none; border-left: none; border-bottom: none;\">&nbsp;</td>
								</tr>
							</table>
						</td>
					</tr> 
			...
			$chari .= "
				<table style=\"border-collapse:collapse;font-family: tahoma; font-size:10px\" width=\"100%\" align=\"right\" border=\"0\">
						<tr>
							<td width=\"50%\" style=\"text-align:center;\"> &nbsp; </td>
							<td width=\"50%\" style=\"text-align:center;\"><b>$kota, ".date('d-m-Y', strtotime($kasir->tglbayar))."</b></td>
						</tr> 
			...
			$judul = "CETAK DOKUMEN JAMINAN NO. KWITANSI : ".$nokwitansi;
			// echo ("<title>$judul</title>");
			$this->M_cetak->mpdf_tiket(80,190,'P', 'A4', $judul, $chari, '.PDF', 10, 10, 10, 2);
		} else {
			header('location:' . base_url());
		}
	}
