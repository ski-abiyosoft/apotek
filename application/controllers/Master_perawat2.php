...
class Master_perawat2 extends CI_Controller
{
  ...
  public function index()
  {
    ...
    if (!empty($cek)) {
      ...
      $data["cabang"] = $cabang;
      $data["list"]   = $this->db->query("SELECT * FROM tbl_dokter WHERE jenispegawai = 2 $kondisi")->result();
      $this->load->view('master/v_master_perawat2', $data);
    } else {
  ...
  public function v_add()
  {
    ...
    if (!empty($cek)) {
      ...
      $data['modul']    = 'PERAWAT';
      $data['submodul'] = 'Master Perawat';
      $data['link']     = 'Master Perawat Entri';
      $data['url']      = 'Master_perawat2';
      $data['tanggal']  = date('d-m-Y');
      $data['akses']    = $akses;
      $this->load->view('master/v_add_masterperawat', $data);
    } else {
      header('location:' . base_url());
    }
  }
  ...
  public function f_add()
  {
    ...
    if (!empty($cek)) {
      ...
      $kodokter     = urut_transaksi('MASTER_PR', 10);
  ...
  public function v_edit($id)
  {
    ...
    if (!empty($cek)) {
      ...
      $this->load->view('master/v_edit_masterperawat', $data);
    } else {
      header('location:' . base_url());
    }
  }
