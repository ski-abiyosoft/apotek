...
class Master_pendidikan extends CI_Controller {
	...
	{
		parent::__construct();
		$this->load->model('M_pendidikan','M_pendidikan');
		$this->load->helper('simkeu_rpt');
		$this->session->set_userdata('menuapp', '1000');
		$this->session->set_userdata('submenuapp', '1109');
	}
	...
	public function index()
	{
		...
		if(!empty($cek))
		{
			$this->load->view('master/v_master_pendidikan');
		} else
		{
	...
	{
		$data = $this->M_pendidikan->get_by_id($id);		
	...
	public function ajax_add()
	{
		...
		$insert = $this->M_pendidikan->save($data);
	...
	public function ajax_update()
	{
		...
		$this->M_pendidikan->update(array('id' => $this->input->post('id')), $data);
		echo json_encode(array("status" => TRUE));
	}

	public function ajax_delete($id)
	{
		$this->M_pendidikan->delete_by_id($id);
		echo json_encode(array("status" => TRUE));
	}
