...
class Master_gudang extends CI_Controller {
	...
	public function index()
	{
		...
		if(!empty($cek))
		{
			$this->load->view('master/v_master_gudang');
		} else
		{
	...
	public function ajax_list()
	{
		...
		foreach ($list as $unit) {
			...
			$row[] = '<a class="btn btn-sm btn-primary" href="javascript:void(0)" title="Edit" onclick="edit_data('."'".$unit->depocode."'".')"><i class="glyphicon glyphicon-edit"></i> </a>
				  <a class="btn btn-sm btn-danger" href="javascript:void(0)" title="Hapus" onclick="delete_data('."'".$unit->depocode."'".')"><i class="glyphicon glyphicon-trash"></i> </a>';
	...
	public function ajax_update()
	{
		...
		$this->M_gudang->update(array('depocode' => $this->input->post('kode')), $data);
		echo json_encode(array("status" => TRUE));
	}
