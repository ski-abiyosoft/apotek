...
class Master_kelasterapi extends CI_Controller {
	...
	{
		parent::__construct();
		$this->load->model('M_kelasterapi','M_kelasterapi');
		$this->load->helper('simkeu_rpt');
		$this->session->set_userdata('menuapp', '1000');
		$this->session->set_userdata('submenuapp', '1201');
	}
	...
	public function index()
	{
		...
		if(!empty($cek))
		{
			$this->load->view('master/v_master_kelasterapi');
		} else
		{
	...
	{
		$list = $this->M_kelasterapi->get_datatables();
		$data = array();
		$no = $_POST['start'];
		...
		$output = array(
						"draw" => $_POST['draw'],
						"recordsTotal" => $this->M_kelasterapi->count_all(),
						"recordsFiltered" => $this->M_kelasterapi->count_filtered(),
						"data" => $data,
				);
	...
	{
		$data = $this->M_kelasterapi->get_by_id($id);		
	...
	public function ajax_add()
	{
		...
		$data = array(
				...
				'apogroup' => 'KELAS_TERAPI',
				
			);
		$insert = $this->M_kelasterapi->save($data);
	...
	public function ajax_update()
	{
		...
		$this->M_kelasterapi->update(array('id' => $this->input->post('id')), $data);
		echo json_encode(array("status" => TRUE));
	}

	public function ajax_delete($id)
	{
		$this->M_kelasterapi->delete_by_id($id);
		echo json_encode(array("status" => TRUE));
	}
