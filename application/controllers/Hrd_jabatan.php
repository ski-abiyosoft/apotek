...
class Hrd_jabatan extends CI_Controller {
	...
	public function index()
	{
		...
		if(!empty($cek))
		{
			$this->load->view('hrd/v_hrd_jabatan');
		} else
		{
	...
	public function ajax_list()
	{
		...
		foreach ($list as $jabatan) {
			...
			$row[] = '<a class="btn btn-sm btn-primary" href="javascript:void(0)" title="Edit" onclick="edit_bank('."'".$jabatan->kode."'".')"><i class="glyphicon glyphicon-pencil"></i> Edit</a>
				  <a class="btn btn-sm btn-danger" href="javascript:void(0)" title="Hapus" onclick="delete_bank('."'".$jabatan->kode."'".')"><i class="glyphicon glyphicon-trash"></i> Hapus</a>';
	...
	public function ajax_update()
	{
		...
		$this->M_jabatan->update(array('kode' => $this->input->post('kode')), $data);
		echo json_encode(array("status" => TRUE));
	}
