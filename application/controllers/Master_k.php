...
class Master_k extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->session->set_userdata('submenuapp', '6101');
	}
	...
	public function index()
	{
		...
		$karyawan   = $this->db->query("SELECT*From tbl_kary_ski where resign='0' ORDER BY pos,namakary ")->result();
		...
		if(!empty($cekk))
		{
			$data  = array(
				'form_id'    => 'absen',
				'form_title' => 'Data Karyawan',
				'cek'        => $cek,
				'karyawan' 	 => $karyawan,
				'status'     => $status,
			);
			$this->load->view('master/v_master_k',$data);
		} else
		{
